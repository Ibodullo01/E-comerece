...
def cart(request):
    order = Order.objects.filter(customer=request.user,
        ...
        order_total_price=Sum('order_item__total_price')
    ).first()
    ...
    if order:
        if order_item:
            if order_item.quantity > 1:
                ...
                order_item.quantity -= 1
                order_item.delete()
            messages.info(request, f'{order_item.product.title} item updated !')
            return redirect('order:cart')
        else:
            messages.info(request, f'{order_item.product.title} this item does not exist!')
            return redirect('order:cart')
    else:
        messages.info(request, f'{order_item.product.title} You not order !')
        return redirect('order:cart')
